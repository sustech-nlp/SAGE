...
from sage.data.spider_eval.process_sql import get_schema, Schema, get_sql

# NOTE: `eval_exec_match` is only used by the full Spider end-to-end evaluator
# (the `evaluate(...)` driver below, when etype in {"all", "exec"}). Import it
# lazily at the call site to keep hardness-only preprocessing lightweight.
...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if etype in ["all", "exec"]:
                # Lazy import — only needed for end-to-end exec evaluation,
                # not for hardness scoring.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', dest='output', type=str, help="the path to the output file")
    ...
    # 使用重定向，将所有的print内容写入到output文件中
    with open(args.output, 'w') as f:
        with contextlib.redirect_stdout(f):  # 重定向print的输出到文件
            evaluate(
                args.gold,
                args.pred,
                args.db,
